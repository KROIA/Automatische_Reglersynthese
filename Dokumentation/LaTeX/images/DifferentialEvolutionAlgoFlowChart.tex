...
\node (mut)      [process, below of=eval] {Mutation};
\node (cross)    [process, below of=mut] {Kreuzung};
\node (sel)      [process, below of=cross] {\begin{tabular}{c}Selektion /\\Fitness evaluieren\end{tabular}};

\node (dec)      [decision, below of=sel, yshift=-0.5cm] {Abbruchbedingung?};
...
\draw [arrow] (eval) -- (mut);
\draw [arrow] (mut) -- (cross);
\draw [arrow] (cross) -- (sel);
\draw [arrow] (sel) -- (dec);
\draw [arrow] (dec) -- node[anchor=east] {Ja} (stop);

% Loopback arrow
\draw [arrow] (dec.west) --++ (-2,0) |- node[pos=0.25, anchor=west] {Nein} (mut.west);
