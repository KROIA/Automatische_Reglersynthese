function fig = tableToFigure(tableOrCsv, widthRatio)
    ...
    if istable(tableOrCsv)
        % Input is a table - convert to cell array format
        splitRows = tableToSplitRows(tableOrCsv);
    elseif ischar(tableOrCsv) || isstring(tableOrCsv)
        % Input is a file path - read CSV
        raw = readlines(tableOrCsv);
