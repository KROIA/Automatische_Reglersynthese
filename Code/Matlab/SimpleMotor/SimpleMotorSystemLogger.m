...
%
% Das Skript kann nach der Ausführung des Modells "SimpleMotorSystem.slx" dazu verwendet werden,
...
labels        = {'Zeit [s]','r [v]','l [v]','e [v]','u [v]','y [v]','u [v]','y [v]','u [v]','y [v]'};
lineStyles    = {'', '-', '-', '-', '-', '-', '-', '-', '-', '-'};
...
saveTableToCSV(result(:,[1 2 3 5 6]) ,  '../../DokuPlotTables/SimpleMotor/Systune_simulinkResult.csv');
saveTableToCSV(result(:,[1 2 3 7 8]) ,  '../../DokuPlotTables/SimpleMotor/Genetic_simulinkResult.csv');
saveTableToCSV(result(:,[1 2 3 9 10]) , '../../DokuPlotTables/SimpleMotor/Differential_simulinkResult.csv');

%saveTableToCSV(result(:,[1 2 3 5 6]) ,  'Systune_simulinkResult.csv');
%saveTableToCSV(result(:,[1 2 3 7 8]) ,  'Genetic_simulinkResult.csv');
%saveTableToCSV(result(:,[1 2 3 9 10]) , 'Differential_simulinkResult.csv');

% -------------------------------------------------------------------------------------------------
function fig = saveTableToPDF(cellArray, pdfFileName, widthRatio)
    exportPath = '../../../../Dokumentation/LaTeX/images/SimpleMotor';
