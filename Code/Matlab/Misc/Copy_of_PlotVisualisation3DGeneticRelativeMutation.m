...
calcZ = @(x, y) -(global_depth * exp(-((x*xStretch - global_x).^2 + (y - global_y).^2) / global_width) + ...
                local1_depth * exp(-((x*xStretch - local1_x).^2 + (y - local1_y).^2) / local1_width) + ...
                local2_depth * exp(-((x*xStretch - local2_x).^2 + (y - local2_y).^2) / local2_width) + ...
                0.15 * ((x*xStretch).^2 + y.^2));
...
hold on;
num_points = 20;  % Number of points in the group
scatter_radius = 0.3;  % Radius of the point cloud

% Generate random points around the specified location
points_x = -1.6 + scatter_radius*2 * (rand(num_points, 1) * 2 - 1);
points_y = 1.7 + scatter_radius*10 * (rand(num_points, 1) * 2 - 1);
