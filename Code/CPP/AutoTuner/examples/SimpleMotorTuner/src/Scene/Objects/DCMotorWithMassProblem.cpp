...
DCMotorWithMassProblem::DCMotorWithMassProblem(const SetupSettings& setupSettings,
...
//#endif
	...
	*/
	...
	m_solverObject->setParametersTestFunc(std::bind(&DCMotorWithMassProblem::agentTestFunction, this, std::placeholders::_1, std::placeholders::_2));
	...
	m_frequencyResponse.setOutputIndex(1);
	//m_frequencyResponse.setOutputIndex(1); // Position output
	//addDrawFunction(
	//	[this](const GameObject&, sf::RenderTarget& target, sf::RenderStates)
	//	{
	//		static int counter = 0;
	//		counter++;
	//		static std::vector<AutoTuner::FrequencyResponse::FrequencyResponsePoint> responseData;
	//		if (counter % 10 == 0)
	//		{
	//			counter = 0;
	//			responseData = m_frequencyResponse.getResponse(m_testSystem.getFeedForwardPart(), 0.01, 100.0);
	//		}
	//		if (responseData.size() > 0)
	//			m_frequencyResponse.drawNyquistPlot(responseData, "DC Motor with Mass - Nyquist Plot");
	//	}
	//);
	
}

void DCMotorWithMassProblem::onAwake()
{
	testCustomPID();
	precalculatePIDWithZieglerNichols();
}
void DCMotorWithMassProblem::precalculatePIDWithZieglerNichols()
{
	std::vector<float> timeData;
	std::vector<float> responseData;
	DCMotorWithMassSystem dcMotorSystem;
	...
	for (float x = 0; x < 1; x += dt)
	{
		...
		responseData.push_back(static_cast<float>(dcMotorSystem.getOutputs()[1]));
...
void DCMotorWithMassProblem::setupPopulation(size_t populationSize, double kp, double ki, double kd, double integralSatturation, double areaRange)
{
...
void DCMotorWithMassProblem::update()
{
	if (m_solverObject)
	{
...
void DCMotorWithMassProblem::createNewStepSequence()
{
	m_stepData = generateRandomStepSequence(m_testSystem.getSystemInputLimit() * 0.8, 10, 0.5, 3.0, 5);
	

}
void DCMotorWithMassProblem::resetPopulation()
{
	if (m_solverObject)
	{
...
void DCMotorWithMassProblem::testBestAgent()
{
	if (m_solverObject)
	{
...
void DCMotorWithMassProblem::setLearningAmount(double learningAmount)
{
	if (m_solverObject)
	{
...
double DCMotorWithMassProblem::getLearningAmount() const
{
	if(m_solverObject)
	{
...
void DCMotorWithMassProblem::setLearningRate(double learningRate)
{
	AutoTuner::GeneticSolver* geneticSolver = dynamic_cast<AutoTuner::GeneticSolver*>(m_solverObject);
...
double DCMotorWithMassProblem::getLearningRate() const
{
	return m_learningRate;
}
void DCMotorWithMassProblem::saveResultsToFile(const std::string& resultName)
{
...
void DCMotorWithMassProblem::setResultsLearningHistoryColors(const sf::Color& color)
{
	setResultsLearningHistoryColors(color, color, color);
}
void DCMotorWithMassProblem::setResultsLearningHistoryColors(const sf::Color& averageColor, const sf::Color& bestColor, const sf::Color& worstColor)
{
...
void DCMotorWithMassProblem::setResultsParameterColors(const sf::Color& color)
{
...
void DCMotorWithMassProblem::setResultsParameterColors(const std::vector<sf::Color>& colors)
{
...
size_t DCMotorWithMassProblem::getParameterCount() const
{
	if (m_solverObject)
	{
...
void DCMotorWithMassProblem::setResultsParameterEnabled(size_t index, bool enabled)
{
...
void DCMotorWithMassProblem::setResultsLearningHistoryEnableColumn(bool enableMin, bool enableAverage, bool enableMax)
{
...
size_t DCMotorWithMassProblem::getResultsStepResponseSingalsCount() const
{
	return m_resultData.stepResponse.responseSignals.size();
}
void DCMotorWithMassProblem::setResultsStepResponseSignalColor(size_t index, const sf::Color& color)
{
...
void DCMotorWithMassProblem::setResultsStepResponseSignalName(size_t index, const std::string& name)
{
	if (index < m_resultData.stepResponse.responseSignals.size())
	{
...
void DCMotorWithMassProblem::setResultsStepResponseSignalLineStyle(size_t index, AutoTuner::CSVExport::LineStyle style)
{
	if (index < m_resultData.stepResponse.responseSignals.size())
	{
...
void DCMotorWithMassProblem::setResultsStepResponseSignalLineThickness(size_t index, int thickness)
{
	if (index < m_resultData.stepResponse.responseSignals.size())
	{
...
void DCMotorWithMassProblem::setResultsStepResponseSignalEnabled(size_t index, bool enabled)
{
	if (index < m_resultData.stepResponse.responseSignals.size())
	{
...
const sf::Color& DCMotorWithMassProblem::getResultsStepResponseSignalColor(size_t index) const
{
	if (index < m_resultData.stepResponse.responseSignals.size())
	{
...
const std::string& DCMotorWithMassProblem::getResultsStepResponseSignalName(size_t index) const
{
	if (index < m_resultData.stepResponse.responseSignals.size())
	{
...
const AutoTuner::CSVExport::LineStyle DCMotorWithMassProblem::getResultsStepResponseSignalLineStyle(size_t index) const
{
	if (index < m_resultData.stepResponse.responseSignals.size())
	{
...
int DCMotorWithMassProblem::getResultsStepResponseSignalLineThickness(size_t index) const
{
	if (index < m_resultData.stepResponse.responseSignals.size())
	{
...
bool DCMotorWithMassProblem::isResultsStepResponseSignalEnabled(size_t index) const
{
	if (index < m_resultData.stepResponse.responseSignals.size())
	{
...
std::vector<double> DCMotorWithMassProblem::getBestParameters() const
{
	if (m_solverObject)
	{
...
//#endif
	...
	size_t counter = 0;
	size_t targetPlotPointCount = 5000;
	size_t plotCountInterval = (targetPlotPointCount > 0) ? (static_cast<size_t>(m_setupSettings.endTime / dt) / targetPlotPointCount) : 1;
...
//#ifdef DISABLE_ERROR_INTEGRAL_WHEN_SATURATED
			...
			if (m_setupSettings.disableErrorIntegrationWhenSaturated)
			{
				...
				double angle = m_testSystem.getOutput();
				...
				bool isLowerSaturated = pidOutput < -actuatorLimit + 0.01;
				if (!((isPositiveSaturated && r < angle) || (isLowerSaturated && r > angle)))
				{
...
//#endif
	...
	if(m_nyquistPlotComponent)
	{
		...
		if (counter % 10 == 0)
		{
			...
			//qDebug() << responseData.gainMargin;
...
std::vector<double> DCMotorWithMassProblem::agentTestFunction(const std::vector<double>& parameters, size_t agent)
{
	...
	if ((m_tuningGoalFactor_gainMargin != 0 || m_tuningGoalFactor_phaseMargin != 0) && m_epochCounter > 100)
	{
		...
		double gainDiff = m_setupSettings.targetGainMargin - gainMargin;
		double phaseDiff = m_setupSettings.targetPhaseMargin - phaseMargin;
		

		losses[3] = gainDiff * gainDiff * m_tuningGoalFactor_gainMargin;
		losses[4] = phaseDiff * phaseDiff * m_tuningGoalFactor_phaseMargin;

		//qDebug() << losses[4] << " phaseDiff: "<< phaseDiff << " phaseMargin: "<< phaseMargin;
		//if(agent == 0)
		//{
		//	qDebug() << "Gain Loss: " << losses[3] << ", Phase Loss: " << losses[4];
		//}
	...
	for (double t = 0; t < endTime; t += dt)
	{
		...
		double angle = agentSystem.getOutput();
		...
		// Check if the controller is not saturating
		double error = agentSystem.getError();
//#ifdef DISABLE_ERROR_INTEGRAL_WHEN_SATURATED
		if (m_setupSettings.disableErrorIntegrationWhenSaturated)
		{
			bool isPositiveSaturated = pidOutput > actuatorLimit - 0.01;
		...
		// Check overshoot
		if ((lastR < angle) && (r < angle) && rWasRising)
		{
			double overshoot = (angle - r) / systemInputLimit;
...
void DCMotorWithMassProblem::printSignalSequenceToConsole(const std::string& name, const std::vector<sf::Vector2<double>>& steps) const
{
...
std::vector<sf::Vector2<double>>  DCMotorWithMassProblem::generateRandomStepSequence(double stepAmplitude, double maxTime, double minStepDuration, double maxStepDuration, size_t stepCount)
{
...
void DCMotorWithMassProblem::setCSVHeader()
{
...
void DCMotorWithMassProblem::logCSVData()
{
	if (m_solverObject)
	{
		...
		/*
		size_t paramIndexCounter2 = 0;
#ifdef PARAMETERLIST_ENABLE_KP
		parameterChanges[paramIndexCounter++].data.push_back(bestParameters[paramIndexCounter2++]);
#else
...
void DCMotorWithMassProblem::testCustomPID()
{
...
void DCMotorWithMassProblem::saveResultDataToFile(const ResultData& resultData, std::string folderPath) const
{
